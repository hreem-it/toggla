...
public class TogglerService {
    ...
    @Inject
    @ConfigProperty(name = "toggler.auto-toggle-refresh-rate.cron", defaultValue = "0/30 * * 1/1 * ? *")
        ...
        if (currentStatus == null) {
            ...
            try {
                if (variationKey != null) {
                    status = togglerGateway.getToggleVariationStatus(key, variationKey);
                } else {
                    status = togglerGateway.getToggleStatus(key);
                }
